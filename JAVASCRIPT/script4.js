...
{
    const element = tds[i];
    element.addEventListener('click', function(e){   //Ajoute un événement sur chaque td
        const tdContent = element.innerHTML;          //Va modifier le contenu
        element.innerHTML = "";                     //Vide le content
        const input = document.createElement("input");      //Créer un input à la place
        input.type = "text";
        input.value = tdContent;                   //Ajoute le contenu modifiable
        const target = e.target;
        target.appendChild(input);            //ajouter un input apres le click
        input.focus();

        
        input.addEventListener("focusout", function(e){         //Ajoute un événement sur chaque input en sortant de celui ci
            const targetInput = e.target.value;
            const inputContent = targetInput.value;
            targetInput.parentElement.innerHTML = inputContent;           
        });

        //evite de vider la cellule sur input lors du click
        input.addEventListener("click", function(e){         //Ajoute un événement sur chaque input en sortant de celui ci
